...
from utils.scanvault_queue import add_to_queue, is_in_queue
...
class RealTimeMonitor:
    ...
    def wait_and_scan_file(self, path):
        ...
        try:
            ...
            # 🔒 FIX RACE CONDITION: Check if file was recently restored
            from Scanning.vault_processor import get_vault_processor
            processor = get_vault_processor()
            if processor.is_recently_restored(norm_path):
                log_message(f"[MONITOR] Skipping recently restored file (race condition prevention): {norm_path}")
                telemetry_inc('race_condition_prevented')
                return
            
            # ✅ QUEUE-BASED SYSTEM: Add to queue instead of direct capture
            # This ensures sequential one-by-one processing without race conditions
            try:
                event_type = 'download_finalized' if rename_followed else 'created'
                
                # Check if already in queue (prevents duplicates from multiple events)
                if is_in_queue(norm_path):
                    log_message(f"[MONITOR] File already in processing queue: {os.path.basename(norm_path)}")
                    return
                
                # Add to queue for sequential processing
                if add_to_queue(norm_path, event_type):
                    log_message(f"[MONITOR] ✅ Added to queue: {os.path.basename(norm_path)}")
                    telemetry_inc('queued_for_processing')
                else:
                    log_message(f"[MONITOR] Already in queue: {os.path.basename(norm_path)}")
                
                # Queue processor will handle: capture → scan → restore (one by one)
                return
            except Exception as e:
                error_msg = str(e)
                # 🛡️ Handle rate limiting gracefully (brute force protection)
                if "Rate limit:" in error_msg or "Burst attack blocked:" in error_msg:
                    telemetry_inc('rate_limit_triggered')
                    # Don't log individual rate limit hits (too noisy), already logged in scanvault.py
                    return  # Exit cleanly - file will be retried later if legitimate
                
                # 🔧 Handle installation mode cases gracefully
                if "Installation mode" in error_msg:
                    if "Threat detected and quarantined" in error_msg:
                        # Malware quarantined during installation mode - this is expected
                        telemetry_inc('install_mode_quarantine')
                        print(f"[INFO] Installation mode: Threat quarantined from {norm_path}")
                    elif "Clean file, not vaulted" in error_msg:
                        # Clean file left in place during installation mode - this is expected
                        telemetry_inc('install_mode_skip')
                        print(f"[INFO] Installation mode: Clean file left in place: {norm_path}")
                    else:
                        print(f"[INFO] Installation mode: {error_msg}")
                    return  # Exit cleanly for installation mode
